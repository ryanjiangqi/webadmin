...
router.beforeEach((to, from, next) => {
	...
	Vue.prototype.appUrl='http://120.77.146.13:8081';
	//前端不需要验证token
	let webPath = ['index', 'product', 'detail', 'webabout'];
	let path = to.path;
	let pathArray = path.split('/'); //以分号拆分字符串
	console.log(pathArray);

	var checkNext = webPath.some((item) => {
		return item == pathArray[1];
	});
